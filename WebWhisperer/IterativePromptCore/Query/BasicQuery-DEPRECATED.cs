...
namespace WebWhisperer.IterativePromptCore.Query
{
    public class Processor
    {
        ...
        public IEnumerable<ITransformation> CreateQuery()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    var moves = transformationCandidate.GetNextMoves(Response).ToList();
                    var nextMove = string.Empty;

                    while (true)
                    {
                        // Print all possible moves for the transformation
                        foreach (var move in transformationCandidate.GetNextMoves(Response))
                        {
                            Console.Write($"{move}; ");
                            moves.Add(move);
                        }
        ...
        {
            var api = new OpenAIAPI(WebWhisperer.IterativePromptCore.Query.Secrets.Credentials.PersonalApiKey);
